...
This module includes a capture template for Org tasks linking to emails.
Pressing =l= on a message in the headers buffer will capture that message. It
can be disabled by setting ~+mu4e-capture-template~ to ~nil~.
